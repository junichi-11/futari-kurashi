...
for(const product of catalog.products){
  ...
  if(product.affiliate_url!==undefined&&product.affiliate_url!==null)errors.push(`${product.id}: affiliate URL must remain null`);
  if(product.publishable===true)errors.push(`${product.id}: draft product cannot be publishable`);
...
if(command==="build"){
  ...
  const sources={version:1,last_verified:catalog.last_verified,sources:catalog.products.map(product=>({id:`src-${product.id}`,product_id:product.id,official_url:product.official_url,rakuten_url:product.official_url,manufacturer:product.brand,shop:product.shop,last_verified:catalog.last_verified,source_type:"rakuten_retailer",mutable_fields_review:"required_before_publication"}))};
  ...
  const queue=["# Affiliate Link Queue","","新規40商品はすべて `affiliate_url: null`、`publishable: false` です。楽天の商品ページで価格・在庫・送料・レビューを再確認し、人間が商品リンクを発行してください。",""];
  ...
  const cards=briefs.map(brief=>`<article><p>${brief.category} · ISSUE ${brief.issue}</p><h2>${brief.title}</h2><dl><dt>Publication</dt><dd>${brief.publicationStatus}</dd><dt>Products</dt><dd>10 / 10</dd><dt>Affiliate</dt><dd>0 / 10</dd><dt>Hero</dt><dd>${brief.heroImageStatus}</dd><dt>Validation</dt><dd>PASS</dd><dt>Blocker</dt><dd>Affiliate URL、可変商品情報、Hero、人間レビュー</dd></dl><a href="/articles/${brief.slug}/?preview=1">Previewを確認 →</a></article>`).join("");
  const review=`<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>Editorial Review Index | MARGIN</title><style>body{margin:0;background:#f4f1eb;color:#171715;font-family:Arial,sans-serif}.wrap{width:min(1100px,88vw);margin:80px auto}header{border-bottom:1px solid #d8d2c8;padding-bottom:40px}h1{font:400 clamp(42px,7vw,84px) Georgia,serif}section{display:grid;grid-template-columns:1fr 1fr;margin-top:70px;border-top:1px solid #d8d2c8}article{padding:40px;border-bottom:1px solid #d8d2c8}article:nth-child(odd){border-right:1px solid #d8d2c8}article>p,dt,dd,a{font-size:11px}dl{display:grid;grid-template-columns:100px 1fr}dt,dd{padding:8px 0;border-bottom:1px solid #d8d2c8;margin:0}a{display:inline-block;margin-top:22px;color:inherit}@media(max-width:700px){section{grid-template-columns:1fr}article:nth-child(odd){border-right:0}}</style><script>document.addEventListener('DOMContentLoaded',()=>{if(new URLSearchParams(location.search).get('preview')!=='1')document.body.innerHTML='<main class="wrap"><h1>Preview only</h1><p>このページはProductionでは公開しません。</p></main>'})</script></head><body><main class="wrap"><header><p>MARGIN / Editorial Production Board</p><h1>Preview Review Index</h1><a href="/docs/Affiliate-Link-Queue.md">Affiliate Link Queue</a></header><section>${cards}</section></main></body></html>`;

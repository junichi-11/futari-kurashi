...
const html = renderArticlePageV3({ ...mock, articleType: "比較・おすすめ記事", target: "新婚・2人暮らし", livingArea: "リビング" }, "https://futari-kurashi.vercel.app", mock.publishedAt, mock.updatedAt);
const checks = {
  ...
  "responsive product images": /class="product-visual">(?:<a[^>]+>)?<img[^>]+srcset="[^"]+ 480w,[^"]+ 800w"[^>]+width="960" height="720" loading="lazy" decoding="async"/.test(html),
  "responsive comparison images": /class="compare-product">(?:<a[^>]+>)?<img[^>]+srcset="[^"]+ 160w,[^"]+ 320w,[^"]+ 480w"[^>]+width="160" height="120" loading="lazy" decoding="async"/.test(html),

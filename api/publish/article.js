...
async function publishHandler(req, res) {
  ...
  const siteOrigin = String(process.env.PUBLIC_SITE_ORIGIN || VERCEL_ORIGIN).replace(/\/$/, ""), now = new Date().toISOString();
  ...
  try {
    ...
    const entry = { articleId: validation.articleId, slug: validation.slug, url: `/articles/${validation.slug}/`, displayTitle: cleanText(normalized.article.displayTitle), seoTitle: cleanText(normalized.article.seoTitle), metaDescription: cleanText(normalized.article.metaDescription), publishedAt, updatedAt, articleType: cleanText(normalized.articleType), target: cleanText(normalized.target), livingArea: cleanText(normalized.livingArea), productCategory: cleanText(normalized.productCategory), editorialThemes: Array.isArray(normalized.editorialThemes) ? normalized.editorialThemes.map(cleanText) : [], mainKeyword: cleanText(normalized.seo?.mainKeyword), productCount: validation.products.length, thumbnail: cardImage(normalized.article.cardImage || normalized.article.coverImage || normalized.article.heroImage || validation.products[0]?.imageUrl), status: "Published" };
